...
const CommandCenter = {
      ...
      <div style="max-width: 1400px; margin: 0 auto; font-family: 'Inter', sans-serif;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; margin-bottom:24px;">
          ...
          <div style="display:flex; gap:10px; flex-wrap:wrap;">
            ...
            <button onclick="CommandCenter.toggleAutoPilot()" id="btn-autopilot" class="btn btn-secondary" style="background:linear-gradient(135deg,#8b5cf6,#6d28d9); color:#fff; border:none; display:flex; align-items:center; gap:6px; padding:10px 18px; font-weight:700; box-shadow:0 4px 20px rgba(109,40,217,0.4); transition:all 0.3s;">
              🤖 Auto-Pilot: OFF
            </button>
  ...
  startLiveFeed() {
    ...
    this.liveInterval = setInterval(() => {
      ...
      // Autonomous AI Engine Logic
      if (this.isAutonomous) {
        this.runAutonomousDecisions();
      } else {
        // Randomly update one zone fill level if NOT autonomous
...
Generated by EcoFlow Command Center v3.0`;
  ...
  toggleAutoPilot() {
    ...
    if (this.isAutonomous) {
      btn.innerHTML = '🤖 Auto-Pilot: <span style="color:#34d399;margin-left:4px;">ON (LIVE)</span>';
      btn.style.boxShadow = '0 0 25px rgba(52,211,153,0.6)';
      btn.style.animation = 'novaPulse 2s infinite';
      Utils.showToast('🤖 NOVA Autonomous Operations Engine Activated', 'success');
      this.logCommand('⚡ [AUTONOMOUS] Core orchestration engine engaged', '#a78bfa');
      if (window.NovaAgent && !window.NovaAgent.isOpen) window.NovaAgent.addBubble("🚀 **Auto-Pilot mode ON!** Ab main city ki routing, emergencies aur dispatch khud sambhal raha hoon. Aap monitor karo!", 'nova');
    } else {
      btn.innerHTML = '🤖 Auto-Pilot: OFF';
      btn.style.boxShadow = '0 4px 20px rgba(109,40,217,0.4)';
      btn.style.animation = 'none';
      Utils.showToast('🛑 Autonomous Mode Deactivated', 'error');
      this.logCommand('🛑 [AUTONOMOUS] Orchestration engine disengaged', '#f87171');
    }
  },

  runAutonomousDecisions() {
    // 1. Monitor bin levels and auto-dispatch
    const criticalZone = this.cityZones.find(z => z.fillLevel >= 88);
    if (criticalZone) {
      criticalZone.fillLevel = Math.max(30, criticalZone.fillLevel - 40); // Auto clear
      this.logCommand(`🤖 [AUTO-DISPATCH] Critical fill detected in ${criticalZone.name}. Auto-dispatched 2 backup trucks. Issue mitigated.`, '#ef4444');
      if (window.NovaAgent) window.NovaAgent.addBubble(`🚨 **Emergency Action:** ${criticalZone.name} mein bin level ${criticalZone.fillLevel + 40}% gaya tha. Maine turant 2 truck bhej diye hain! ✅`, 'nova');
    }

    // 2. Auto-resolve oldest pending alert
    const unresolved = this.emergencyAlerts.find(a => !a.resolved);
    if (unresolved && Math.random() > 0.6) {
      unresolved.resolved = true;
      this.renderAlertList();
      this.logCommand(`🤖 [AUTO-RESOLVE] AI automatically resolved SLA alert: ${unresolved.id} in ${unresolved.zone}`, '#34d399');
    }

    // 3. Autonomous Carbon Trade
    if (Math.random() > 0.8) {
      this.logCommand(`🤖 [AUTO-TRADE] High market price detected (₹1,310). Autonomously sold 24 Verra VCS credits.`, '#10b981');
    }

    // Continue normal heatmap fluctuations but skewed downwards (optimizing)
    const zoneIdx = Math.floor(Math.random() * this.cityZones.length);
    this.cityZones[zoneIdx].fillLevel = Math.max(10, Math.min(99, this.cityZones[zoneIdx].fillLevel + (Math.random() > 0.7 ? 2 : -3)));
  },

  logCommand(msg, color) {
    const container = document.getElementById('command-log');
    if (!container) return;
    const time = new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false });
    const entry = document.createElement('div');
    entry.style.cssText = `color:${color}; padding:3px 0; border-bottom:1px solid rgba(255,255,255,0.04);`;
    entry.innerHTML = `<span style="color:#475569; margin-right:8px;">${time}</span><strong>${msg}</strong>`;
    container.prepend(entry);
    while (container.children.length > 30) container.removeChild(container.lastChild);
  }
};
